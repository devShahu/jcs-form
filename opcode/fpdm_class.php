...
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    //@@@@@@@@@
		...
		*@param string $mode a choice between 'safe','check','verbose' or 'verbose_level'
		...
		**/
		...
		//--------------------------------
           ...
           $CountLines = count($entries);
			...
			$xref_table=0;
			$trailer_table=0;
			$n=0; //Position of an object, in the order it appears in the pdf file
			$stream=array();
		...
		//-------------------------
		   ...
		   //..so that this regexp can do its job without annoyances 
			if(preg_match_all("/(T|V)\s*\(([^\)]+)\)\s*\/(T|V)\s*\(([^\)]+)\)/", $content,$matches, PREG_PATTERN_ORDER)) {
				...
				if($this->verbose) $this->dumpEntries($fields,"FDF has no fields",false);
			
			if($this->verbose) $this->dumpEntries($fields,"FDF parsed",false);
